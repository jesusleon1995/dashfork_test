...
const char * const BITCOIN_CONF_FILENAME = "hustcoin.conf";
const char * const BITCOIN_PID_FILENAME = "hustcoind.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "hustcoin" is a composite category enabling all HustCoin-related debug output
            if(ptrCategory->count(string("hustcoin"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\HustCoinCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\HustCoinCore
    // Mac: ~/Library/Application Support/HustCoinCore
    // Unix: ~/.hustcoincore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "HustCoinCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/HustCoinCore";
#else
    // Unix
    return pathRet / ".hustcoincore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty hustcoin.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override hustcoin.conf
